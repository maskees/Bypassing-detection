...
from models.road_sign_model import load_road_sign_checkpoint
from road_sign_data import CLASS_NAMES, make_road_sign_datasets
from train_road_sign_model import collate_batch


def main():
    parser = argparse.ArgumentParser(description="Evaluate a trained road-sign checkpoint.")
    parser.add_argument("--checkpoint", default="saved_models/road_sign_resnet34.pth")
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--output", default="results/road_sign_eval.json")
    args = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"
    _, val_ds = make_road_sign_datasets(return_display=False)
    loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, collate_fn=collate_batch)
    model, checkpoint = load_road_sign_checkpoint(args.checkpoint, device=device)
    model.eval()
