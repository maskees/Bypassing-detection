...
from models.road_sign_classifier import RoadSignClassifier
...
def make_class_weights(counts, device):
    ...
    weights = [total / max(counts[idx], 1) for idx in range(len(CLASS_NAMES))]
...
def run_epoch(model, loader, optimizer, device, class_weights, train):
    model.train(train)
    total_loss = 0.0
    correct = 0
    total = 0
...
def main():
    parser = argparse.ArgumentParser(description="Train a cropped road-sign classifier.")
    parser.add_argument("--annotations-dir", default="annotations")
    parser.add_argument("--images-dir", default="images")
    parser.add_argument("--output", default="saved_models/road_sign_crop_resnet34.pth")
    parser.add_argument("--backbone", choices=["resnet18", "resnet34", "efficientnet_b0"], default="resnet34")
    ...
    train_ds, val_ds = make_road_sign_crop_datasets(
    ...
    class_weights = make_class_weights(class_counts(train_ds), device)
    print(f"Class weights: {[round(x, 3) for x in class_weights.detach().cpu().tolist()]}")

    best_acc = -1.0
    best_metrics = None
    start = time.time()
    for epoch in range(1, args.epochs + 1):
        train_metrics = run_epoch(model, train_loader, optimizer, device, class_weights, train=True)
        val_metrics = run_epoch(model, val_loader, None, device, class_weights, train=False)
        scheduler.step()
        ...
        if val_metrics["accuracy"] >= best_acc:
            ...
            best_metrics = {"train": train_metrics, "validation": val_metrics}
            save_checkpoint(args.output, model, epoch, best_metrics, args)
            print(f"  saved best checkpoint -> {args.output}")

    elapsed = time.time() - start
    metrics_path = Path("results") / "road_sign_crop_training_metrics.json"
